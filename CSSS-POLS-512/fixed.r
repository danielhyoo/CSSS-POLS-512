...
n <- 100												#Set the number of students
t <- 5													#Set the number of obs per student
beta = 0.75												#Set the slope of grades
sigmaAlpha <- 0.7										#Set the sd of the random effects
sigma <- 0.2											#Set the sd of the error term

rXm <- c(-0.7,.7)										#Set the mean of hours of study
sdX <- 0.3												#Set the sd of hours of study

collist <- brewer.pal(9,"Set1")							#Designate color list Set1
while (length(collist)<n) collist <- c(collist,collist)	#Repeat list of colors to fit obs
nt <- n*t												#Set the number of total obs
x <- matrix(rep(NA,nt),n,t)								#Create an empty matrix of n rows and t cols
y <- matrix(rep(NA,nt),n,t)								#Create an empty matrix of n rows and t cols
alpha <- rnorm(n,sd=sigmaAlpha)							#Generate the random effects, drawn from a normal dist

for (i in 1:n) {
  x[i,] <- rnorm(t, mean=(runif(1)*(rXm[2]-rXm[1])-rXm[1]), sd=sdX)		#Generate the hours of study for each studen
  y[i,] <- alpha[i] + beta*x[i,] + rnorm(t,sd=sigma)					#Generate the grades for each student
}

# Make traces
BWdataTraces <- dataTraces <- linesTraces <- vector("list",n)			#Create empty vectors of n list objects for all traces
for (i in 1:n) {
  dataTraces[[i]] <- pointsTile(x=x[i,], y=y[i,], col=collist[i], pch=16, plot=1)	#Tell tile to trace t colored points for each student
  BWdataTraces[[i]] <- pointsTile(x=x[i,], y=y[i,], col="black", pch=16, plot=1)	#Tell tile to trace t BW points for each student
  yfit <- c( min(x[i,])*beta + alpha[i],
             max(x[i,])*beta + alpha[i])  
  linesTraces[[i]] <- linesTile(x=c(min(x[i,]),max(x[i,])),y=yfit,col=collist[i], plot=1) #Tell tile to plot a colored line for each student
}
 yfit <- c( min(x)*beta,
             max(x)*beta)  
