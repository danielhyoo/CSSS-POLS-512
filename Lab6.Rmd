...
Recall that we can remove fixed effects by differencing (different from first differencing):
...
This is because $\bar{y}_i$ is computed using all the past $y$'s. This is correlated with $\bar{\epsilon}_{it}$, which is computed using all the past $\epsilon$'s \newline

Specifically, this creates bias in the LDV (think conditional mean zero assumption). If the other regressors are correlated with the LDV, then their coefficients may also be seriously biased. \newline
...
Furthermore, 
  ...
  + The bias increases as $\beta$ decreases \bigskip

  + The bias increases as $\phi$ increases \bigskip

  + Small $N$ is not the problem. Small T is the problem \bigskip
...
Recall that an instrumental variable must fulfill two conditions: 1) it is correlated with $x$ (relevance); 2) it is uncorrelated with $\epsilon$ (exogeneity). It must influence $y$ only through $x$. 
...
Recall that the following condition holds under Gauss-Markov. 
...
Intuition: GMM attempts to find the $\boldsymbol{\beta}$ that makes this true.
...
$$\boldsymbol{S}_{xy}=n^{-1}\sum^n_{t=1}\boldsymbol{x}_t y_{t} \;\;\; \text{and} \;\;\; \boldsymbol{S}_{zx}=n^{-1}\sum^n_{t=1}\boldsymbol{z}_t x_{t}$$
We solve for
...
\textbf{Anderson-Hsiao estimator}: uses the twice and third lagged levels as instruments. \newline

\textbf{Arellano-Bond Difference GMM}: uses $\Delta y$ as the outcome and all available lagged levels as instruments in each period. \newline

\textbf{Arellano-Bover/Blundell-Bond System GMM}: adds the available lagged differences as instruments. \newline
